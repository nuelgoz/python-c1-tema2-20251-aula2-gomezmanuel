...
def test_post_text(client):
    response = client.post(
        "/text", data="Este es un texto de prueba", content_type="text/plain"
    )
...
def test_post_html(client):
    response = client.post(
        "/html", data="<h1>Prueba HTML</h1>", content_type="text/html"
    )
    assert response.status_code == 200
...
def test_post_image(client):
    ...
    # Send the image
    response = client.post("/image", data=img_bytes, content_type="image/png")
...
def test_post_binary(client):
    ...
    response = client.post(
        "/binary", data=binary_data, content_type="application/octet-stream"
    )
